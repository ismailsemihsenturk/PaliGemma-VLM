...
class KVCache():
    ...
    def num_items(self)-> int:
        if len(self.key_cache) == 0:
            ...
            # The shape of the key_cache is [Batch_Size, Num_Heads_KV, Seq_Len_ Head_Dim]
...
class GemmaRMSNorm(nn.Module):
    ...
    def __init__(self, dim: int, eps: float = 1e-6):
        ...
        self.weight = nn.Parameter(torch.zeros(dim)) # learnable gamma parameter
...
class GemmaRotaryEmbedding(nn.Module):
    ...
    def __init__(self,dim, max_position_embeddings=2048, base=10000, device=None):
        ...
        self.dim=dim # it is set to head_dim
        self.max_position_embeddings = max_position_embeddings
        self.base = base
...
def apply_rotary_pos_emb(q,k,cos,sin,unsqueeze_dim=1):
    ...
    # Apply the formula (34) of the Rotary Positional Encoding Paper https://arxiv.org/pdf/2104.09864
...
class GemmaAttention(nn.Module):
    ...
    def __init__(self, config: GemmaConfig, layer_idx:Optional[int]=None):
        ...
        # Size of the embedding vector of each token
        self.hidden_size = config.hidden_size
        # Number of the attention heads
        self.num_heads = config.num_attention_heads
        # Number of the how many dimensions each head will work with in the multi-head attention which is the part of the entire embedding of each token
        self.head_dim = config.head_dim
        self.num_key_value_heads = config.num_key_value_heads
        self.num_key_value_groups = self.num_heads // self.num_key_value_heads
        # How many position we can encode in the positional encoding using the rotary positional encoding
        self.max_position_embeddings = config.max_position_embeddings
        # What is the best frequency of the rotary positional encoding.
        ...
        # IN MULTI HEAD ATTENTION THE BOTTLENECK OF COMPUTATION IS NOT HOW MANY DOT PRODUCTS WE ARE DOING BUT HOW MUCH TIME IT TAKES TO COPY THE MEMORY FROM THE HIGH BANDWIDTH MEMORY TO LOCAL MEMORY SO THAT CORES CAN WORK WITH IT.
        # Cores fast enought to compute but gpu is not fast enough to copying stuff around comparing to cores.
        # Difference of the group query attention is that we have less head for the keys and values which results in a smaller projection for the embedding of each token when it's used as keys and value.
        # So group of heads of queries is sharing some heads for the keys so that they don't need to copy for different heads these dimensions but they can share the already copied ones.
    ...
    def forward(
        ...
        # [Batch_Size, Num_Heads_Q, Seq_Len_ Head_Dim]
        query_states = query_states.view(bsz,q_len,self.num_heads,self.head_dim).transpose(1,2)
        key_states = key_states.view(bsz,q_len,self.num_key_value_heads,self.head_dim).transpose(1,2)
        # [Batch_Size, Num_Heads_KV, Seq_Len, Head_Dim]
        value_states = value_states.view(bsz,q_len,self.num_key_value_heads,self.head_dim).transpose(1,2)

        # [Batch_Size, Seq_Len, Head_Dim], [Batch_Size, Seq_Len_ Head_Dim]
        ...
        # [Batch_Size, Num_Heads_Q, Seq_Len_Q, Seq_Len_KV] 
        # * 
        # Batch_Size, Num_Heads_KV, Seq_Len_KV, Head_Dim] 
        # -> 
